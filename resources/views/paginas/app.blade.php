...
<head>
    ...
    <!-- Favicon -->
    <link rel="icon" type="image/png" href="/logo-tc.png">
    <link rel="shortcut icon" href="/logo-tc.png">
    ...
    <style>
        ...
        /* Tema claro → logo color indigo */
        .logo-tc-light {
            filter: brightness(0) saturate(100%) invert(37%) sepia(60%) saturate(700%) hue-rotate(215deg) brightness(90%);
            opacity: 0.90;
        }

        /* ── Logo TaskCollab dashboard (grande, translúcido) ────── */
        .logo-tc-dashboard {
            width: 200px;
            height: auto;
            object-fit: contain;
        ...
        .logo-tc-dashboard-dark {
            filter: brightness(0) invert(1);
            opacity: 0.18;
        }

        .logo-tc-dashboard-light {
            filter: brightness(0) saturate(100%) invert(37%) sepia(60%) saturate(700%) hue-rotate(215deg) brightness(90%);
            opacity: 0.15;
        }
    </style>
...
<body class="{{ $bgBody }} font-sans min-h-screen flex transition-colors duration-200">
    ...
    <aside id="sidebar"
        class="{{ $bgSidebar }} w-64 border-r flex flex-col fixed inset-y-0 left-0 z-40 transform -translate-x-full md:translate-x-0 transition-transform duration-200 ease-in-out">
        <div class="p-4 border-b {{ $borderMuted }} flex items-center justify-between h-14">
            <a href="{{ route('dashboard') }}"
                class="flex items-center space-x-2.5 hover:opacity-80 transition-opacity">
                <img src="{{ asset('logo-tc.png') }}" alt="TaskCollab Logo"
                    class="logo-tc-sidebar {{ $isDark ? 'logo-tc-dark' : 'logo-tc-light' }}">
                <span class="font-bold text-indigo-600 dark:text-indigo-400 tracking-tight text-xl">TaskCollab</span>
            </a>
            <button onclick="toggleSidebar()"
                class="md:hidden p-1 rounded-lg text-slate-400 hover:text-red-500 hover:bg-slate-100 dark:hover:bg-zinc-800 cursor-pointer">
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-6">
            ...
            <div>
                <h3 class="px-3 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Mis
                    Espacios</h3>
                <ul class="space-y-0.5">
                    @forelse($paginasPrivadas as $p)
                        <li class="group relative">
                            ...
                            <div id="menu-{{ $p->id }}"
                                class="hidden absolute right-2 top-[90%] w-36 rounded-xl shadow-xl py-1 border z-50 space-menu-dropdown {{ $isDark ? 'bg-[#121214] border-zinc-800 text-zinc-300' : 'bg-white border-slate-200 text-slate-700' }}">
                                <button onclick="openEditSpaceModal(event, {{ $p->id }}, '{{ addslashes($p->titulo) }}')"
                                    ...
                                    <svg class="w-3.5 h-3.5 mr-2 opacity-70" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <button
                                    onclick="openDeleteSpaceModal(event, {{ $p->id }}, '{{ addslashes($p->titulo) }}', '{{ route('paginas.destroy', $p->id) }}')"
                    ...
                    @empty
                        <li class="text-xs text-slate-400 dark:text-zinc-500 px-3 py-1.5 italic">No hay espacios creados
                        </li>
                    @endforelse
                </ul>
            </div>
            ...
            <div>
                <h3 class="px-3 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-2">
                    Espacios Colaborativos</h3>
                <ul class="space-y-0.5">
                    ...
                    @empty
                        <li class="text-xs text-slate-400 dark:text-zinc-500 px-3 py-1.5 italic">Sin espacios compartidos
                        </li>
                    @endforelse
                </ul>
            </div>
        ...
        {{-- Handle de resize --}}
        <div id="sidebar-resize" class="absolute top-0 right-0 bottom-0 w-1.5 cursor-col-resize z-50 group">
            <div
                class="absolute inset-y-0 right-0 w-full group-hover:bg-indigo-500/40 group-active:bg-indigo-500/60 transition-colors rounded-full">
            </div>
        </div>
    </aside>

    <div id="sidebar-overlay" onclick="toggleSidebar()"
        class="fixed inset-0 bg-black/40 z-30 hidden md:hidden backend-blur-xs"></div>
    ...
    <div id="main-content" class="flex-1 flex flex-col md:pl-64 min-w-0 transition-all duration-200">
            ...
            <div class="flex items-center space-x-2.5">
                ...
                <div class="relative inline-block text-left">
                    <button onclick="toggleUserMenu()"
                        ...
                        <svg class="w-3 h-3 text-slate-400 transition-transform" fill="none" viewBox="0 0 24 24"
                    ...
                    <div id="user-menu"
                        ...
                        <a href="{{ route('profile.edit') }}"
                            ...
                            <svg class="w-3.5 h-3.5 mr-2 text-slate-400" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="m-0 p-0">
                            ...
                            <button type="submit"
                                ...
                                <svg class="w-3.5 h-3.5 mr-2 text-red-500" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <main class="flex-1 p-6 md:p-12 w-full max-w-full block clear-both">
            {{-- Logo translúcido de fondo en el dashboard --}}
            <div class="w-full">
                @yield('content')
            </div>
        </main>
    ...
    <div id="edit-space-modal"
        class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/50 backdrop-blur-xs">
        <div
            class="w-full max-w-md rounded-2xl p-6 border shadow-2xl transition-all {{ $isDark ? 'bg-[#121214] border-zinc-800 text-zinc-200' : 'bg-white border-slate-200 text-slate-800' }}">
            <h3 class="text-base font-bold mb-4">Editar espacio</h3>
            <form id="edit-space-form" method="POST" action="">
                @csrf
                @method('PATCH')
                <div class="mb-4">
                    <label for="edit-space-titulo"
                        class="block text-xs font-semibold uppercase tracking-wider mb-1.5 {{ $isDark ? 'text-zinc-500' : 'text-slate-400' }}">Nombre
                        del espacio</label>
    ...
    <div id="delete-space-modal"
        ...
        <div
            ...
            <p class="text-xs mb-4 {{ $isDark ? 'text-zinc-400' : 'text-slate-500' }}">Esta acción no se puede deshacer.
                Se eliminarán permanentemente todas las tareas y subpáginas de <span id="delete-space-name"
