...
@section('header-actions')
    <button onclick="alert('Módulo de invitación en desarrollo')"
            class="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold px-3 py-1.5 rounded-xl transition cursor-pointer flex items-center space-x-1.5 shadow-sm shadow-indigo-600/10">
...
@endphp
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        ...
        <div class="{{ $bgCard }} rounded-2xl border p-4">
            <div class="flex items-center justify-between mb-4 pb-2 border-b {{ $borderMuted }}">
                ...
                <span class="text-xs font-semibold px-2 py-0.5 rounded-full {{ $isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500' }}">
            ...
            <div class="space-y-2.5">
                @forelse($pagina->tareas->where('status', 'todo') as $tarea)
                    <div class="{{ $isDark ? 'bg-[#18181b] border-zinc-800/80 hover:border-zinc-700' : 'bg-slate-50 border-slate-200 hover:border-slate-300' }} border p-3 rounded-xl flex items-center justify-between transition-all shadow-xs group">
                        <div class="flex items-center space-x-3 min-w-0 flex-1">
                            <form action="{{ route('tareas.update', $tarea->id) }}" method="POST" class="flex items-center m-0 p-0">
                                @csrf
                                @method('PATCH')
                                <input type="checkbox" onchange="this.form.submit()" {{ $tarea->status === 'done' ? 'checked' : '' }}
                                       class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-0 cursor-pointer">
                            </form>
                            <span class="text-sm font-medium truncate {{ $textTitle }} {{ $tarea->status === 'done' ? 'line-through opacity-45' : '' }}">
                                {{ $tarea->title }}
                            </span>
                        </div>
        ...
        <div class="{{ $bgCard }} rounded-2xl border p-4">
            ...
            <div class="space-y-2.5">
                @forelse($pagina->tareas->where('status', 'done') as $tarea)
                    <div class="{{ $isDark ? 'bg-[#18181b]/60 border-zinc-900' : 'bg-slate-50/60 border-slate-200' }} border p-3 rounded-xl flex items-center justify-between transition-all group">
                        <div class="flex items-center space-x-3 min-w-0 flex-1">
                            <form action="{{ route('tareas.update', $tarea->id) }}" method="POST" class="flex items-center m-0 p-0">
                                @csrf
                                @method('PATCH')
    ...
    <div id="taskModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-xs p-4">
        <div class="{{ $isDark ? 'bg-[#121214] border-zinc-800' : 'bg-white border-slate-200' }} border w-full max-w-md rounded-2xl shadow-2xl overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-150">
            ...
            <div class="p-4 border-b {{ $borderMuted }} flex justify-between items-center">
                <h3 class="text-base font-bold {{ $textTitle }} flex items-center space-x-2">
                    <svg class="w-4 h-4 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5M11 3h5a2 2 0 012 2v5m-9 7h3l12-12a1.5 1.5 0 00-2-2l-12 12v3z" />
                    </svg>
